...
export default function EngineBay() {
  ...
  const sectionSchema = sections.find(s => s.slug === 'engine');
  ...
  useEffect(() => {
    async function loadDraft() {
      ...
      try {
        ...
        const sectionData = draft.sections.find(s => s.slug === 'engine');
        if (sectionData) {
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Typography color="text.secondary" paragraph>
          Check for leaks, examine belts, hoses and battery condition.
        </Typography>
        ...
        {!draft ? (
          ...
          <>
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
              ...
              <Button
                variant="outlined"
                onClick={() => nav(`/rust?draftId=${draftId}`)}
              >
                Back
              </Button>
            </Stack>
