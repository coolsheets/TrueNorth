...
export default function RoadTest() {
  ...
  const sectionSchema = sections.find(s => s.slug === 'road');
  ...
  useEffect(() => {
    async function loadDraft() {
      ...
      try {
        ...
        const sectionData = draft.sections.find(s => s.slug === 'road');
        if (sectionData) {
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Typography color="text.secondary" paragraph>
          Evaluate vehicle start-up, acceleration, braking and handling.
        </Typography>
        ...
        {!draft ? (
          ...
          <>
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
              ...
              <Button
                variant="outlined"
                onClick={() => nav(`/interior?draftId=${draftId}`)}
              >
                Back
              </Button>
            </Stack>
