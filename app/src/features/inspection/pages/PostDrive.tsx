...
export default function PostDrive() {
  ...
  const sectionSchema = sections.find(s => s.slug === 'post');
  ...
  useEffect(() => {
    async function loadDraft() {
      ...
      try {
        ...
        const sectionData = draft.sections.find(s => s.slug === 'post');
        if (sectionData) {
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Typography color="text.secondary" paragraph>
          After the road test, check for any fresh leaks, unusual odors, or starting issues.
        </Typography>
        ...
        {!draft ? (
          ...
          <>
            ...
            <Stack direction="row" spacing={2} sx={{ mt: 4 }}>
              ...
              <Button
                variant="outlined"
                onClick={() => nav(`/road?draftId=${draftId}`)}
              >
                Back
              </Button>
            </Stack>
