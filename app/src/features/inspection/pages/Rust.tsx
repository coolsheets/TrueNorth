...
export default function Rust() {
  ...
  const sectionSchema = sections.find(s => s.slug === 'rust');
  ...
  useEffect(() => {
    async function loadDraft() {
      ...
      try {
        ...
        const sectionData = draft.sections.find(s => s.slug === 'rust');
        if (sectionData) {
    ...
    <Stack spacing={3}>
      <Card>
        ...
        <Typography color="text.secondary" paragraph>
          Check for rust issues on frame rails, cab corners, rockers, and bumper mounts - critical in Canadian climate.
        </Typography>
